...
- Scripts: `saveScript(key, { dir, name, source, description?, internal?, previousName? })` writes `<dir>/lib/<name>.ts` through `writeLibraryFunction` (name checked with `libraryNameProblem`, source non-empty and of any size, `PACK_CONFLICT` when another function has the name; `previousName` renames by removing the old file after the new one is written; `internal: true` writes the `// @internal` first line and `EditorScript.internal` reports it back, offered as an "Internal helper" checkbox). A source that does not hold one function for the character to call (one function expression, or a module exporting one) is still saved — the author is mid-edit — and comes back with `problem` set. `removeScript(key, dir, name)` deletes the file; `scriptTemplate()` returns `libraryFunctionTemplate()`.
