...
A library holding an internal helper renders as two scopes and calls it twice; the outer object, built
last, is the one `sdk.lib` keeps, so the helper is out of reach from both names. `transpile(code, language, { prelude })` places it between
