...
4. Install globals in the isolate:
  ...
  `internalNames` carries the library's `@internal` helpers (docs/spec/core.md "Internal helpers"). The runner passes the bootstrap a fourth argument, `restrictInternals`, true only when `request.context.trigger.kind === 'llm'`; when it is, those functions are wrapped so that a call made while no library function is on the stack throws `lib.<name> is an internal helper …`, and a call made from inside another library function goes through. Every other trigger (behaviour hooks, event handlers, `code` timers, the Sandbox tab) gets the functions unwrapped. The wrapper keeps `String(lib.<name>)` the saved source, so the source still reads back in the isolate and a library function handed to `sdk.events.on` still serialises as itself (§4). It is a visibility boundary, not a sandbox one: overlapping library calls raise the depth for each other, and every function is the character's own code with the same permissions either way.
...
undefined (`sdk.lib` still carries the module's two methods).
...
- prelude: `lib.double` defined by the prelude is callable from the code, and through `sdk.lib.double` too (`sdk.lib === lib`, with `register`/`unregister` on it and the host seeing only those two); a failure in the code after a three-line prelude still reports the user's line; a failure inside a library function shows `at lib.<name> (library:…)`; a prelude syntax error is reported as a library problem without a caret into the action
