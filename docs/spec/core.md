...
**LibraryService** (`services/library.ts`): the character's own function library (`sdk.lib`). Functions are files in the installed pack, `characters/<id>/lib/<name>.ts` (docs/spec/pack.md "Function library"), read by the pack loader into `LoadedCharacter.library`; the service is constructed with `LibraryPacks` (`PackService.getLoaded` + `reloadCharacterLibrary`) and reads `list`, `functions` (with sources, for the prompt; `LibFunction { name, source, description?, bytes, updatedAt }` with `updatedAt` = file mtime), `source` and `preludeFor` straight from the loaded pack, so a function the author shipped is there from the first install and a function the character defines survives sessions and restarts (a reinstall replaces the folder, see PackService). `define(target, name, fn, opts?)` validates the name (`libraryNameProblem` from `@rp/pack`: `^[a-zA-Z_$][\w$]*$`, ≤ 64 chars, no reserved words, no `__proto__`), the caps (50 functions, 128 KiB in total, counted against the current library; no per-source cap) and that the source parses as a single function expression (`functionSourceProblem` from `@rp/pack`: esbuild `transformSync`, never `new Function`), then writes the file with `writeLibraryFunction` (atomic: temp + rename; `// <description>` first line when given) and rescans through `PackService.reloadCharacterLibrary`; `remove` deletes the file (`removeLibraryFunction`) and rescans. Nothing is kept in state any more (`LIB_STATE_KEY` remains only for the migration and to keep a not-yet-migrated key out of `<state>`). `preludeFor(packId, characterId, opts?)` renders `const lib = Object.freeze({\n  "<name>": (<source>),\n  …\n});` (`const lib = Object.freeze({});` when empty), cached per character (one entry per `internals` value) and invalidated on every write and on pack changes. The prelude reaches every run as `CodeRunRequest.prelude`: `ChatService.runLlmTurn` sets `TurnInput.prelude` for `ActionLoop.runAction`, and `BehaviourRunner` (`run`/`runScript`: timer code, event handlers, lifecycle hooks) asks its `prelude` option. `ChatService` also removes `lib.functions` from the `<state>` block — the library has its own section.

**Internal helpers.** A library file whose first line is `// @internal` (docs/spec/pack.md "Function library") is the author's plumbing, not the character's: `list` leaves it out, `source` reports `NOT_FOUND` for it, `define` refuses its name (`INVALID_ARGUMENT`, `lib.<name> is reserved by this pack …`) so the character cannot replace it, `remove` returns `false` and deletes nothing, and `PromptBuilder` leaves it out of `<library>` (`functions` still returns it — it is the prelude's input). With one in the library `buildPrelude` emits two scopes instead of one:

```js
const lib = (() => {
  const lib = Object.freeze({ "<helper>": (…), "<public>": (…) });  // every function; shadows the outer binding
  return Object.freeze({ "<public>": lib["<public>"] });            // what the running code sees
})();
```

so a library function calling `lib.<helper>(…)` resolves against the full object while `lib.<helper>` is `undefined` in the code around the prelude. `preludeFor(…, { internals: true })` skips the split and is asked only for scripts the pack itself ships: `BehaviourRunner.runFor` (the behaviour hooks) passes `{ internals: true }`, while `runScript` (a `code` timer, an event handler — code the character wrote) and `ChatService` (actions) take the split prelude. This is a visibility boundary, not a sandbox one: a character that defines a library function of its own can still reach the inner object from inside it.
