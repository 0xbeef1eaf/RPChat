...
- *Action trimming.* `keepActionDetailFor` (default 2) keeps the code and results of the most recent assistant messages that acted; older messages contribute only their visible text. `transcriptToMessages` drops a message's `tool_use` and `tool_result` blocks together, so a result is never orphaned from its call, in both tool and fenced mode.
