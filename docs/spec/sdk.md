...
- `block(patterns | { deny } | { allow }, { durationMs?, redirect?, reason? }): Promise<{ id; mode; expiresAt; patterns }>` (no duration = until lifted; a bare array or `{ deny }` names the pages kept shut, `{ allow }` the only ones that may open — top-level navigations only, so an allowed page's own assets always load; 127.0.0.1/localhost/browser pages refused in a denylist and always let through by an allowlist; one allowlist at a time), `unblock(id)`, `blocks(): Promise<BrowserBlock[]>`, `clearBlocks()`.
