...
export class CapabilityDispatcher implements CapabilityInvoker {
  ...
  async invoke(call: CapabilityCall): Promise<CapabilityResult> {
    ...
    try {
      ...
      if (verdict === 'deny') {
        const reason = this.permissions.denialReason ? await this.permissions.denialReason(context.packId, module, method) : 'switched off under Settings → Permissions';
        return fail(
          new RpError('PERMISSION_DENIED', `sdk.${module}.${method} is not available: ${reason}`, { module, method, reason }),
          'denied',
        );
      }
      if (verdict === 'prompt') {
