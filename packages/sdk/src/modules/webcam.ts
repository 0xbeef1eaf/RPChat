 ...
 * with source: 'home'. Requires the 'webcam' capability; the user can switch it off entirely
...
- Both calls save into \`webcam/\` in your home folder and return an \`AssetRef\` with \`source: 'home'\`. Home files are **not** pack assets: \`sdk.media.*\` and \`sdk.wallpaper.set\` take pack assets only. Use \`sdk.files.open/read/list/delete\` on the path, and \`sdk.files.homePath()\` to tell the user where the file is.
...
await sdk.files.open(shot.path);
