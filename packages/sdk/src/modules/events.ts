...
export const eventsModule: CapabilityModuleSpec = {
  ...
  version: '1.0.0',
  title: 'Events',
  summary: 'React to things happening on the PC (idle, window/song changes, time, files, widget messages, media clicks, the chat window coming and going) with a handler that runs when they occur.',
  permission: 'trusted',
 ...
 * directly or wake yourself with sdk.llm.wake(). Handlers run on their own, alongside the
 * conversation rather than in front of it, so a slow one never keeps you from replying; each
 * subscription still handles its own events one at a time, in order. Subscriptions belong to this
 * session (max 30) and survive app restarts. Nothing polls: this is the right way to "wait for" something.
 */
interface EventsApi {
  /**
...
- Filters: \`time\` \`{ hour, minute?, weekday? }\`; \`user-idle\` \`{ idleMs }\` (without it, as soon as the host counts the user as idle; with it, once they have been away that long); \`window-changed\`/\`app-launched\` \`{ app?, title? }\` (substring); \`battery-low\` \`{ percent }\`; \`file-added\` \`{ dir?, ext? }\`; \`widget-message\` \`{ widgetId }\`; \`browser-navigated\` \`{ url?, title? }\` (substring; data \`{ tabId, url, title }\`, only while the browser extension is connected); \`media-clicked\` / \`media-closed\` \`{ mediaId?, asset?, reason? }\` (exact; data \`{ mediaId, asset, packId, kind }\`, closed adds \`reason\`: 'click' | 'timeout' | 'ended' | 'api' | 'error') — the clicks on images you showed with \`sdk.media\`. \`guard-attempt\` \`{ kind?, target?, command?, blocked? }\` (kind/blocked exact, target/command substring; data \`{ kind: 'ipc' | 'config' | 'signal' | 'ptrace' | 'exec', target, command, pid, blocked }\`) — the session guard (Linux, rpchatd) saw the user's own terminal, keybind or picker try to reach the compositor/shell IPC, edit the wallpaper config or kill/trace rpchat; \`blocked\` is true only in enforce mode, and one event per target reaches you at most every 10 s. \`chat-shown\` / \`chat-hidden\` (no filter; data \`{ hiddenMs? }\` / \`{ shownMs? }\`) — the rpchat window itself became visible or went away (the tray, closing it to the tray, a notification click), with how long it had been away or up; minimising it is neither. Interaction events (widget messages, media clicks/closes, avatar clicks, the chat window appearing and going) are never debounced: every click counts. One that arrives while its handler is still running is queued behind it, so a subscription handles its own events in order, one at a time — a slow handler delays its own next event, nothing else.
