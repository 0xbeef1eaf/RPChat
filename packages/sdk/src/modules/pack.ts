...
interface PackApi {
   ...
   * @returns Matching assets, best matches first (more matching tags). Assets that match equally well are
   *   shuffled on every call, so taking the first result gives you a different one each time; pass a
   *   bigger limit and pick yourself only when you want to see the whole set.
...
- \`findAssets\` returns equally good matches in a random order, so \`(await sdk.pack.findAssets({ anyTags: ["cozy"] }))[0]\` is a fresh pick each time rather than always the same file; \`listAssets\` is sorted by path, so shuffle that one yourself.
...
\`\`\``,
  methods: {
    ...
    findAssets: { description: 'Find assets by tags, kind or text (equal matches in random order).' },
