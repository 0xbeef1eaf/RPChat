...
const EXPECTED: Record<string, { permission: string; methods: string[] }> = {
  ...
  browser: {
    permission: 'pack',
    methods: [
      'open', 'status', 'tabs', 'openTab', 'activate', 'close', 'navigate', 'back', 'forward', 'reload', 'read', 'query', 'click', 'type', 'scroll', 'screenshot', 'find',
      'block', 'unblock', 'blocks', 'clearBlocks', 'imageEffect', 'clearImageEffects', 'setHomePage', 'homePage', 'bookmarks', 'searchBookmarks', 'addBookmark', 'removeBookmark',
      'eval', 'history', 'historyVisits', 'recentHistory',
    ],
  },
...
const OVERRIDES: Record<string, { prompt?: string[]; dangerous?: string[] }> = {
  ...
  browser: { dangerous: ['open', 'openTab', 'close', 'navigate', 'click', 'type', 'screenshot', 'block', 'imageEffect', 'setHomePage', 'addBookmark', 'removeBookmark', 'eval'] },
