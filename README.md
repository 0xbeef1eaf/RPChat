...
```bash
sudo resources/system/install.sh          # or native/rpchatd/install.sh from a checkout
```

It creates the `rpchat` group, installs the udev rule and service, sets the app
to start on login, unpacks an AppImage into `/opt/rpchat/current` (the *system install*:
root-owned, updated by the daemon without a password, previous version kept for
`install.sh --rollback`; `--no-system-install` keeps the AppImage), and prepares `/etc/rpchat` for `policy.json`, a root-owned file that
...
sessions so their own terminals, keybind scripts and pickers cannot edit the wallpaper config,
run a wallpaper client (`awww img …`) or kill RPChat — while RPChat itself may; attempts reach
characters as the `guard-attempt` event, and `enforce` blocks them. Reaching the compositor/shell
...
The same installer writes the browser extension policy for **Settings → Browser**, one
...
The repository is public, so the release feed of `0xbeef1eaf/RPChat` is read
anonymously: there is nothing to sign in to and no token to store. The
**AppImage** checks about 30 s
...
is also applied on quit. A **system install** (`/opt/rpchat/current`, what the
system-integration installer makes of an AppImage) downloads the same way and then
hands the file to the `rpchatd` daemon, which verifies the checksum, unpacks it as
your user, swaps it in as root, keeps the old version under `/opt/rpchat/previous`
