...
describe('buildApi', () => {
  ...
  it('wires push events with unsubscribe', () => {
    ...
    api.app.onPolicyChange((snapshot) => got.push(snapshot));
    ...
    ipc.emit(IPC_EVENT_CHANNELS.policyChanged, { managed: [] });
    expect(got).toEqual([{ type: 'status' }, { requestId: 'r' }, { promptId: 'p' }, { type: 'close-all' }, { state: 'ready' }, { managed: [] }]);
    off();
    offPerm();
