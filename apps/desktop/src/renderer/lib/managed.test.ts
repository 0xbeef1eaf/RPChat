...
describe('isManaged', () => {
  const managed = ['autonomy', 'maxInputLockMs', 'permissions.functionAllow.system', 'web.allowlist'];
  it('matches exact paths and parents', () => {
    ...
    expect(isManaged(managed, 'permissions.functionAllow.system')).toBe(true);
  ...
  it('does not match children of a managed leaf, siblings, or prefixes without a dot', () => {
    expect(isManaged(managed, 'permissions.functionAllow')).toBe(false);
    expect(isManaged(managed, 'permissions.functionAllow.media')).toBe(false);
  ...
  it('managedChildren lists forced entries under a path', () => {
    expect(managedChildren(managed, 'permissions.functionAllow')).toEqual(['system']);
