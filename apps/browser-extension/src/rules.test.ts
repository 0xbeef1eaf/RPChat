...
import { describeRule, dnrRulesFor, expiredRules, isProtectedPattern, nextExpiry, normalisePatterns, parsePattern, patternToRegex, readTable, urlMatchesPattern } from './lib/rules.js';
...
describe('block patterns → DNR rules', () => {
  ...
  it('turns every pattern into one main_frame redirect rule with consecutive ids', () => {
    ...
    expect(rules[0]).toMatchObject({ priority: 1, action: { type: 'redirect', redirect: { url: 'chrome-extension://abc/blocked.html?rule=r1' } }, condition: { resourceTypes: ['main_frame'], isUrlFilterCaseSensitive: false } });
...
describe('rule table and expiry', () => {
  const table: RuleTable = {
    nextRuleId: 5,
    rules: [
      { id: 'a', patterns: ['a.test'], ruleIds: [1], createdAt: '2026-01-01T00:00:00.000Z', expiresAt: '2026-01-01T01:00:00.000Z' },
      { id: 'b', patterns: ['b.test'], ruleIds: [2, 3], createdAt: '2026-01-01T00:00:00.000Z', expiresAt: '2026-01-01T03:00:00.000Z', by: 'Mira', redirect: 'https://calm.test/' },
      { id: 'c', patterns: ['c.test'], ruleIds: [4], createdAt: '2026-01-01T00:00:00.000Z' },
    ],
  };
  ...
  it('sanitises a stored table and describes rules without DNR internals', () => {
    ...
    expect(readTable({ rules: [{ id: 'x', patterns: ['x.test'], ruleIds: [9] }, { bogus: true }] })).toEqual({ nextRuleId: 10, rules: [{ id: 'x', patterns: ['x.test'], ruleIds: [9] }] });
    expect(readTable(table)).toEqual(table);
    expect(describeRule(table.rules[1]!)).toEqual({ id: 'b', patterns: ['b.test'], redirect: 'https://calm.test/', expiresAt: '2026-01-01T03:00:00.000Z', by: 'Mira', createdAt: '2026-01-01T00:00:00.000Z' });
    expect(describeRule(table.rules[2]!)).toEqual({ id: 'c', patterns: ['c.test'], createdAt: '2026-01-01T00:00:00.000Z' });
  });
});
